...
public class AppRepository {
    ...
    private void countDatabaseEntries() {
        ...
        List<RecipeAndInstructions> recipeAndInstructions = mDatabase.recipeAndInstructionsDao().getRecipesAndInstructions().getValue();
        if (recipeAndInstructions != null && recipeAndInstructions.size() > 0) {
            for (RecipeAndInstructions recipes: recipeAndInstructions) {
                totalRecipes++;
                ...
                List<Step> steps = recipes.getSteps();
                if (steps != null && steps.size() > 0) {
                    for (Step step: steps) {
                        totalSteps++;
                    }
        ...
        Log.i(TAG, "Database counts: Recipes -> " + totalRecipes + ", Ingredients -> " + totalIngredients + ", Steps -> " + totalSteps);
    }
}
